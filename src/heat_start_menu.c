...
// --BG-GFX--
static const u32 sStartMenuTiles[] = INCBIN_U32("graphics/heat_start_menu/orange_start.4bpp.lz");
static const u32 sStartMenuTilemap[] = INCBIN_U32("graphics/heat_start_menu/orange_start.bin.lz");
static const u32 sStartMenuTilemapSafari[] = INCBIN_U32("graphics/heat_start_menu/bg_safari.bin.lz");
static const u16 sStartMenuPalette[] = INCBIN_U16("graphics/heat_start_menu/orange_start.gbapal");
...
void HeatStartMenu_Init(void) {
  ...
  if (GetSafariZoneFlag() == FALSE) { 
    ...
    if (menuSelected == 0 || menuSelected == MENU_POKETCH || menuSelected == MENU_SAVE) {
...
static void HeatStartMenu_CreateSprites(void) {
  ...
  if (FlagGet(FLAG_SYS_POKEDEX_GET) == TRUE) {
    ...
    sHeatStartMenu->spriteIdBag     = CreateSprite(&gSpriteIconBag, x, y2, 0);
    sHeatStartMenu->spriteIdPoketch = CreateSprite(&gSpriteIconPoketch, x, y3 + 6, 0);
    sHeatStartMenu->spriteIdTrainerCard = CreateSprite(&gSpriteIconTrainerCard, x, y4 + 6, 0);
    sHeatStartMenu->spriteIdSave    = CreateSprite(&gSpriteIconSave, x, y5 + 3, 0);
